...
pub fn follows_diff_range(
    l: &[i64],
    diff_range: Range<i64>,
            ...
            let failing_pair = l.windows(2).skip(1).find_map(|w| {
                ...
                if diff_range.contains(&diff) && (allow_eq || diff != 0) {
                    if ordering == FollowRangeResult::Fluctuates && diff != 0 {
                        ordering = if diff < 0 {
                            FollowRangeResult::Decreasing
                        } else {
                            FollowRangeResult::Increasing
                        };
                        None
                    } else if enforce_one_way
                        && (ordering == FollowRangeResult::Increasing && diff < 0)
...
pub fn all_combos_remove_one<T>(l: &[T]) -> impl Iterator<Item = impl Iterator<Item = &T>> {
    (0..l.len()).map(|exclude| {
        l.iter()
            .enumerate()
            .filter_map(move |(i, e)| Some(e).filter(|_| i != exclude))
    })
}
